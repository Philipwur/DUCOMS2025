...
function flux(U::Array{Float64,2}, p::SWEParameters)
    h, q = eachcol(U)
    u = q ./ h
    return [q, q .* u + 0.5 .* p.g .* h.^2]
end

# Rusanov (local Lax-Friedrichs) numerical flux
function rusanov_flux(U_L::Array{Float64,2}, U_R::Array{Float64,2}, p::SWEParameters)
    h_L, q_L = eachcol(U_L)
    h_R, q_R = eachcol(U_R)
    u_L = q_L ./ h_L
    u_R = q_R ./ h_R
...
function swe_dae_residual!(residual, du, u, p::SWEParameters, t)
    ...
    if p.bc_type == :periodic
        ...
        # MUSCL + RK2
        U_L, U_R = muscl_reconstruct([h_ext q_ext])
        flux = rusanov_flux(U_L, U_R, p)
